...
def Object_tracking(YoloV3, video_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors='', Track_only = []):
    ...
    while True:
        ...
        tracker.predict()
        tracker.update(detections, vid.get(cv2.CAP_PROP_POS_MSEC), original_image)
...
Object_tracking(yolo, video_path, "detection.mp4", show=False, iou_threshold=0.1, rectangle_colors=(255,0,0), Track_only = ["person"])
